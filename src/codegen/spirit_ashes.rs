...
//
// use generated_module::spirit-ashes.schema;
//
// fn main() {
//     let json = r#"{"answer": 42}"#;
//     let model: spirit-ashes.schema = serde_json::from_str(&json).unwrap();
// }

use serde::{Serialize, Deserialize};

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SpiritAshesSchema {
    /// Strenghts of the Spirit Ash. Each briefly described with minimum amount of words.
    ...
    summary: String,
    /// Specific name of the summoned spirit(s). Rarely differs from Item name.
    summon_name: String,
    /// The number of spririts this Spirit Ash summons.
    summon_quantity: Option<i64>,
    /// Array of Rune costs to upgrade to each level, +1 starting at position 0.
    upgrade_costs: Vec<i64>,
    /// Glovewort the Spirit Ash upgrades with.
    upgrade_material: SpiritAshUpgradeMaterial,
}
...
/// An enumeration.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum SpiritAshUpgradeMaterial {
    #[serde(rename = "Ghost Glovewort")]
    GhostGlovewort,
    #[serde(rename = "Grave Glovewort")]
    GraveGlovewort,
}
